...
*/
		...
		if (setsize > bestsize) {
			bestsize = setsize;
			for (j=1;j<=bestsize;j++) bestset[j] = nbestset[j];
		}

	}


	for (j=1;j<=bestsize;j++) printf("%d ",bestset[j]);
	printf ("\nSize = %d", bestsize);
	check();
...
/* 	printf("maxind(%d,%d,array,%d)\n",top,goal,depth); */
	if (top <= 1) {
		...
		if (depth > nbestsize) {
			nbestsize = depth;
			if (top == 1) nset[nbestsize] = array[top];
			for (i=1;i<=startsize;i++) nbestset[i] = set[i];
			for (i=1;i<=nbestsize;i++) nbestset[startsize+i] = nset[i];
			/* check(); */
